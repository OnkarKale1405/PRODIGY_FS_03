...
app.post("/signIn", async (req, resp) => {
    try {
        ...
        const token = jwt.sign({ username: req.body.username, email: req.body.email, contact_no: req.body.contact_no }, secretKey, { expiresIn: '3d' });
        console.log({ token });
...
const auth = (req, resp, next) => {
    ...
    if (!token) {
        resp.status(403).send("Login first!!");
    }
    try {
        const decode = jwt.verify(token, "123secret")
        console.log(decode);
    }
    catch (err) {
        console.log(err);
